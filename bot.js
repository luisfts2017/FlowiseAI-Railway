    ...
    const res = await openai.chat.completions.create(
      {
		...
		temperature: 0.35,        // Respostas mais controladas
        top_p: 0.9,               // Controla diversidade
        frequency_penalty: 0.4,   // Reduz repetição
        presence_penalty: 0.2,    // Incentiva respostas menos previsíveis
...
// ══════════════════════════════════════════════════════════════
// 🔒 FILA POR USUÁRIO — CONTROLE DE CONCORRÊNCIA (v4.4)
// Garante que mensagens do mesmo número nunca rodem em paralelo
// ══════════════════════════════════════════════════════════════

const filasUsuarios = new Map();

async function enfileirarPorUsuario(numero, handler) {
  if (!filasUsuarios.has(numero)) {
    filasUsuarios.set(numero, Promise.resolve());
  }

  const filaAtual = filasUsuarios.get(numero);

  const novaFila = filaAtual
    .catch(() => {})
    .then(() => handler());

  filasUsuarios.set(numero, novaFila);

  try {
    await novaFila;
  } finally {
    if (filasUsuarios.get(numero) === novaFila) {
      filasUsuarios.delete(numero);
    }
  }
}
...
async function iniciarBot() {
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    try {
      let resposta;
      await enfileirarPorUsuario(numero, async () => {
        resposta = await processarMensagem(numero, texto);
      });
